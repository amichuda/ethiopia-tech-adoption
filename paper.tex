...
Figure \ref{fig:theta_delta_raw} shows a plot of the comparative advantages and returns for the cropcut model with controls. We can see here that returns are very high for the $(0,0,1)$ trajectory, but with an exceedingly negative comparative advantage. Conversely, all trajectories that adopted in the first wave have a positive comparative advantage, but negative returns. This is indicative of either negative sorting (consistent with the negative $\phi$ parameter that we found with the weak identification results) or with a systematic problem in adoption in the first year. 
...
This is illustrated in Figure \ref{fig:when_adopt}, where we calculate an aggregation for comparative advantage and returns whether a household adopted in each wave. We can see here that adopting in the first wave is consistent with a positive comparative advantage, but negative returns. This is repeated, but less strongly in wave 2. This means that for waves 1 and 2, households have gains to adopting, but actually receive negative returns to doing so. This is indicative of the fact that there are barriers to those that adopted in waves 1 and 2, or perhaps, that returns from improved seed are not as high as expected. This relationship is reversed in the third wave, albeit both comparative advantage and returns are close to 0. Figure \ref{fig:when_adopt} provides a curious trend across time to adoption: it would seem that negative returns to adoption reduce across time, but adoption-specific advantages in households also get smaller.
...
\begin{figure}
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{p{0.9\textwidth}} 
            \begin{tablenotes}
                  \small
                  \item Note: Author's calculations using ESS. The figures shows the aggregate impact from Table \ref{tbl:unres} using on which period a person adopted as the basis for the aggregation.
            \end{tablenotes}
        \end{tabular}
    \end{table}
\end{figure}

Figure \ref{fig:traj_sum} provides another aggregation of returns and comparative advantage using the times a household adopted hybrid maize. Adopting 0 times leads to negative comparative and advantage and negative returns makes sense. Those that do not adopt would not receive benefits from adopting. However, those that adopt once during waves 1-3, see a positive comparative advantage, but negative returns. This same is also starkly true for those that adopted three times. For those that adopted twice, it would seem that comparative advantage is negative and there are negative returns to adoption. The lack of a pattern in Figure \ref{when_adopt} seems to suggest that the number of times adopting is not as conclusive as \emph{when} households adopted. Specifically, later adoption periods lead to smaller negative comparative advantages and less negative returns. This suggests that the value of adopting is getting higher, and that this may be due to the introduction of newer and more effective seed.
...
The previous results highlight the fact that the definition of hybrid matters. Even when households 
