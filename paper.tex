...
%   Assistant Research Professor at the 
    ...
    \singlespacing
			\noindent
   Household surveys often rely on self-reports of hybrid seed use, which are prone to measurement error. This context exhibits considerable heterogeneity when estimating productivity impacts. Using a three-year panel, we estimate heterogeneous returns to adoption from self-reports of hybrid seed use and find results driven by non-classical measurement error due to seed misclassification. We leverage information on DNA-fingerprinting collected over the same areas in 2018 and find that heterogeneity in the quality of the genetic material masks positive returns to adoption, with the highest returns found for farmers using higher-purity, drought-tolerant, and newly released varieties. When we account for misclassification in our initial heterogeneity analysis, we find that farmers that have the largest potential to benefit from adoption are constrained in their ability to do so. Our research points to the need for policies that expand the accessibility of improved seed and the need for mechanisms to verify its quality.
			% Despite large investments in developing new maize varieties, evidence shows that few farmers consistently adopt them. Average effects on yields mask considerable heterogeneity. Misclassification of seed is a crucial problem in these contexts, as non-classical measurement error can lead to biased effects. We study misclassification in the context of heterogeneous impacts to adoption. We use a three-year panel to estimate heterogeneous returns to adoption from self-reports of hybrid seed use and find results driven by bias due to the seed misclassification. We leverage unique information on maize DNA-fingerprinting collected over the same areas in 2018. Misclassification in self-reported hybrid seed adoption masks heterogeneity in the quality of the genetic material categorized as improved varieties. Accounting for seed misclassification, we find that heterogeneity in the actual quality of the genetic material masks positive returns to adoption, with the highest returns found for farmers using higher-purity germplasm, drought-tolerant maize, and newly released varieties. When we account for misclassification in our heterogeneity analysis, we find that farmers that have the largest potential to benefit from adoption are constrained in their ability to do so. The wide dispersion of older and genetically diluted varieties, for which poorer farmers may be paying a premium points to the need for policies that expand the accessibility of improved seed and verify its quality.
...
% Given the ubiquity of improved seed varieties over the course of decades of access in Ethiopia, 
        ...
        \vspace*{-4em}
            \begin{table}[H]
                ...
                \begin{tabular}{p{1\textwidth}} 
                    \begin{tablenotes}
                          \small
                          \item Note: Author's calculations using ESS. Mean and standard deviation for each variable shown. Standard deviation in parentheses. Maize farmers only. Parcel size, yield and distance variables winsorized at the 1\% level.
                    \end{tablenotes}
                \end{tabular}
        \end{table}  
\newpage
...
\input{results/tables/misclass_95purity.tex}
\vspace*{-2em}
    \begin{table}[H]
        \centering
        \begin{tabular}{p{1\textwidth}} 
